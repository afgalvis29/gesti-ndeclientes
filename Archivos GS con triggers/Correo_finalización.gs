function Correofinalizacion() {
  ...
  var plantilla = HtmlService.createTemplateFromFile('fin');
  ...
  if (nHojaDatos == nHojaActiva && filaActiva > filasencabezado && colActiva == colCondicion && valorActivo == valorcondicion) {
    ...
    MailApp.sendEmail({
      ...
      bcc: "jaristizabal@propiedata.com, abarragan@propiedata.com, mortiz@propiedata.com, achacon@propiedata.com, contabilidad@propiedata.com"
    });

    // Marca la columna 14 como "Enviada"
    celdaActiva.getSheet().getRange(filaActiva, 42).setValue("Enviada");
  }
}
